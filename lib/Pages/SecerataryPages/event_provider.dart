...
class EventProvider with ChangeNotifier {
  ...
  void loadEvents() async {
    ...
    if (_selectedEvents.isEmpty || !_events.containsKey(_selectedDay)) {
      setSelectedDay(
          _selectedDay); // Explicitly refresh the selected day's events
    }
  ...
  void addEvent(Event event, DateTime selectedDay) async {
    final normalizedDate =
        selectedDay.toIso8601String().split('T')[0]; // Get only the date part
  ...
  void updateEvent(Event updatedEvent, DateTime date) async {
    ...
    if (_selectedEvents.isNotEmpty &&
        _selectedEvents[0].date == updatedEvent.date) {
  ...
  void removeEvent(Event event, DateTime date) async {
    final normalizedDate =
        date.toIso8601String().split('T')[0]; // Get only the date part
